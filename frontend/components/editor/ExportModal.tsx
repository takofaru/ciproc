...
import { compressImageCipAsync } from "@/lib/utils/compression"
  ...
  const handleExport = async () => {
    ...
    try {
      if (format === "cip") {
        ...
        // Standard JPG/PNG canvas export
      ...
      // Delay closing modal slightly so they see completion
      setTimeout(() => {
              ...
              <div className="flex flex-col gap-2">
                    ...
                    <button
                      ...
                      className={`py-2.5 rounded-lg border text-xs font-bold transition-all cursor-pointer ${
                        format === fmt
                          ...
                          : "border-zinc-800 bg-[#0d1117] text-zinc-400 hover:border-zinc-700 hover:text-zinc-300"
                      }`}
                    >
                ...
                {format === "cip" && (
                  <p className="text-[10px] text-zinc-500 mt-1 leading-relaxed">
                    Custom compressed format implementing Quantization, Run-Length Encoding, LZW, Huffman, and block-based Arithmetic coding.
                  </p>
                )}
              </div>
              ...
              {format !== "png" && (
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">
                      {format === "cip" ? "Color Compression" : "Quality"}
                    </label>
                  ...
                  <div className="flex justify-between text-[10px] text-zinc-500">
                    ...
                    <span>Lossless Colors (High Quality)</span>
                  </div>
                </div>
              )}
            </>
        ...
        {!isExporting && (
          <div className="modal-footer border-zinc-800 flex justify-end gap-2 px-5 py-4">
            ...
            <button
              ...
              className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold transition-colors disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
            >
              Export
            </button>
          </div>
