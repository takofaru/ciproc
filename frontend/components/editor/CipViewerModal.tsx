      ...
      <div className="modal-box max-w-lg border-zinc-800 bg-[#161b22] text-zinc-100 shadow-2xl">
        <div className="modal-header border-zinc-800 flex items-center justify-between px-5 py-4">
          <h3 className="text-sm font-semibold uppercase tracking-wider text-zinc-300">
            Import & Decompress Custom (.cip) File
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="modal-body px-5 py-5 flex flex-col gap-4">
          {/* File selector when no result and not decompressing */}
          {!isDecompressing && !result && (
            <div
              onClick={triggerUpload}
              className="border-2 border-dashed border-zinc-800 hover:border-blue-500/50 bg-[#0d1117] rounded-xl py-10 px-4 text-center cursor-pointer transition-all flex flex-col items-center justify-center gap-3 group"
            >
              ...
              <div>
                ...
                <p className="text-xs text-zinc-600 mt-1">Files are decompressed entirely client-side</p>
              </div>
              <input
                ref={fileInputRef}
                type="file"
          ...
          {/* Progress bar during decompression */}
          {isDecompressing && (
          ...
          {/* Reconstructed image and statistics view */}
          {result && (
            <div className="flex flex-col gap-5">
              <div className="relative rounded-lg overflow-hidden border border-zinc-800 bg-[#0d1117] p-2 flex justify-center items-center">
                <img
                  src={result.dataUrl}
                  alt="Reconstructed"
                  className="max-h-[260px] w-auto object-contain rounded shadow"
                />
              </div>
          ...
          <div>
            {result && (
              <button
                ...
                className="px-4 py-2 rounded-lg text-xs font-medium border border-zinc-800 text-zinc-400 hover:text-zinc-300 hover:border-zinc-700 transition-colors cursor-pointer"
              >
                Clear File
              </button>
            )}
