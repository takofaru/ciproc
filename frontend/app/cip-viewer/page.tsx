...
import { decompressImageCipAsync } from "@/lib/utils/compression"
...
export default function CipViewerPage() {
  ...
  const decompressFile = async (file: File) => {
    ...
    setResult(null)
    setProgress({ percentage: 0, stage: "Reading file bytes..." })

    try {
      const arrayBuffer = await file.arrayBuffer()
      const bytes = new Uint8Array(arrayBuffer)

      // Decode header quickly to read dimensions and palette size
      ...
      const decompressed = await decompressImageCipAsync(bytes, (p) => {
        setProgress(p)
      })
          ...
          >
            ...
            <div>
              <p className="text-base font-bold text-zinc-200">
                Drag and drop your .cip file here
              </p>
            ...
            <div className="max-w-md text-[11px] text-zinc-600 mt-4 leading-relaxed border-t border-zinc-800/60 pt-4">
              Decodes and reconstructs Ciproc Image Packages using a 5-tier lossless logic chain:
              Quantization, RLE, LZW, Huffman frequencies, and Stream-based Arithmetic Coding.
            </div>
            <input
              ref={fileInputRef}
              type="file"
          ...
          <div className="grid md:grid-cols-3 gap-8 items-start">
            ...
            <div className="md:col-span-2 flex flex-col gap-4">
              <div className="rounded-2xl border border-zinc-800 bg-[#161b22] p-4 flex justify-center items-center shadow-lg relative min-h-[300px]">
                ...
                <img
                  src={result.dataUrl}
                  alt="Decompressed Reconstructed"
                  className="max-h-[450px] max-w-full object-contain rounded shadow-2xl z-10 border border-zinc-800"
                />
              </div>
            ...
            <div className="flex flex-col gap-6">
              <div className="bg-[#161b22] border border-zinc-800 rounded-2xl p-5 flex flex-col gap-6 shadow-lg">
                ...
                <div className="w-full bg-[#0d1117] border border-zinc-800 rounded-xl p-3 flex flex-col gap-1.5">
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-zinc-400">
                    ...
                    Header magic matches "CIP"
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-zinc-400">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                    Decoded range exact bit-lossless
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2.5">
                <button
                  ...
                  className="w-full py-3 px-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs transition-colors flex items-center justify-center gap-2 shadow cursor-pointer"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                ...
                <button
                  ...
                  className="w-full py-3 px-4 rounded-xl border border-zinc-800 bg-[#161b22] hover:border-zinc-700 hover:text-zinc-200 text-zinc-400 text-xs font-bold transition-colors cursor-pointer"
                >
                  Load Different File
                </button>
              </div>
